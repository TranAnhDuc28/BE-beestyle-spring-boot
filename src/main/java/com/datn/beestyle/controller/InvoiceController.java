...
//    }
    @Autowired
    public InvoiceController(InvoicePDFExporter invoicePDFExporter) {
    ...
    public ResponseEntity<byte[]> previewInvoice(@PathVariable Long orderId) {
        try {
            ...
            invoicePDFExporter.exportInvoice(orderId, outputStream); // Xuất file PDF
    ...
    public ResponseEntity<byte[]> generateInvoice(@PathVariable Long orderId) {
        try {
            ...
            logger.info("Request to generate invoice: {}", orderId);
